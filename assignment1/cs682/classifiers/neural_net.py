...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # shape (N, C).                                                             #
    #############################################################################
    #Compute output after the first hidden layer
    h1_output = np.dot(X,W1) + b1
    #Apply ReLU to the output of first hidden layer
    h1_afterActivation = np.maximum(h1_output,0)
    #Use ReLU's output as input to the second layer and compute its output to
    #get final scores
    scores = np.dot(h1_afterActivation,W2) + b2
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    dW2 = np.dot(np.transpose(h1_afterActivation),scores)
    
    #Gradient for b1
    db2 = np.sum(scores,axis = 0)
    
    #Gradient for W1
    #Before Finding the gradient of W1, we need to find backPropagate to Relu layer 
    #input
    d_outputrelu = np.dot(scores, W2.T)
    d_inputrelu = np.where(h1_afterActivation<=0,0,d_outputrelu)
    
    # Gradient for W1
    dW1 = np.dot(X.T, d_inputrelu)
    
    # Gradient for b1
    db1 = np.sum(d_inputrelu, axis=0)
  ...
  def predict(self, X):
    ...
    #Label is the maximum value of score
